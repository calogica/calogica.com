...
### Import Libraries
We'll make use of a couple of popular packages in Python for this project, so let's make we `pip install` and import them first:
...
So, instead, we use content-less dataframe, via `.head(0)` to force the creation of an empty table. In this example, we also specify to replace the table if it already exists.
